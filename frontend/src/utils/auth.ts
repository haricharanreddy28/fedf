...
export const loginUser = async (email: string, password: string, captchaAnswer: string, captchaToken: string): Promise<{ success: boolean; message: string; user?: User; token?: string }> => {
  try {
    const response = await api.post('/auth/login', { email, password, captchaAnswer, captchaToken });
